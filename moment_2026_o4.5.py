...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, config: MOMENTConfig):
        ...
        # Variable embedding to distinguish between different water parameters
        self.variable_embedding = nn.Parameter(
            torch.randn(1, config.n_vars, config.d_model) * 0.02
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, int]:
        ...
        # Add variable embedding (same for all positions)
        x = x + self.variable_embedding.unsqueeze(2)
...
class POTThresholder:
    ...
    def fit(self, scores: np.ndarray) -> float:
        ...
        # Guard against empty or all-NaN scores
        scores_flat = scores_flat[~np.isnan(scores_flat)]
        if scores_flat.size == 0:
            logger.warning("Empty or all-NaN scores passed to POT fit. Using default threshold 0.5")
            self.threshold = 0.5
        else:
...
class MOMENTAnomalyDetector:
    ...
    def __init__(self, config: Optional[MOMENTConfig] = None):
        ...
        # Model will be initialized lazily after we know the actual n_vars from data
        self.model = None
        ...
        logger.info(f"MOMENTAnomalyDetector initialized on {self.config.device}")

    def _build_model(self):
        """Build or rebuild the model with current config (especially n_vars)"""
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Update config with actual number of variables
        self.config.n_vars = len(valid_cols)

        # Build the model now that we know the actual n_vars
        self._build_model()
    ...
    def _fit_threshold(self, train_loader: DataLoader):
        ...
        # Re-create a loader without drop_last to ensure all samples are used
        ...
        with torch.no_grad():
            for batch_data, batch_orig in threshold_loader:
                batch_data = batch_data.to(self.config.device)
                output = self.model(batch_data)
        ...
        if all_scores.size == 0:
            logger.warning("No anomaly scores collected for threshold fitting. Using default threshold.")
            self.thresholder.threshold = 0.5
            self.thresholder.threshold_history.append(0.5)
        else:
    ...
    def load(self, path: str):
        ...
        save_dict = torch.load(path, map_location=self.config.device)
