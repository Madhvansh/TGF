...
def load_moment_pretrained(config: MOMENTConfig) -> Tuple[MOMENT, bool]:
    """
    ...
    """
    model = MOMENT(config)
    encoder_loaded = False  # Track whether encoder has pre-trained weights
    ...
    if config.use_pretrained:
        try:
            ...
            encoder_loaded = encoder_matched > 10  # T5 has ~200+ encoder keys
            logger.info(f"Pre-trained weight transfer: {matched} matched, "
                        f"{skipped} kept random init "
                        f"(encoder keys: {encoder_matched})")
            if not encoder_loaded:
                logger.warning(
                    "Encoder weights did not transfer (architecture mismatch).\n"
                    "This is expected -- momentfm uses T5 encoder, our wrapper "
                    "uses nn.TransformerEncoder.\n"
                    "The encoder has pre-trained weights inside MOMENTPipeline "
                    "but they can't be mapped to our architecture."
                )
            ...
            try:
                encoder_keys = _load_weights_from_huggingface(model, config)
                encoder_loaded = encoder_keys > 10
    ...
    return model, encoder_loaded


def _load_weights_from_huggingface(model: MOMENT, config: MOMENTConfig) -> int:
    """
    Fallback: Load weights directly from HuggingFace hub with intelligent key mapping.

    The pre-trained safetensors uses momentfm key names (T5 encoder).
    Our model uses nn.TransformerEncoder. The encoder architectures are
    FUNDAMENTALLY DIFFERENT (T5 has separate q/k/v, ours has in_proj_weight),
    so encoder weights CANNOT be transferred. We transfer what we can:
    RevIN, patch embedding, mask token, reconstruction head.

    Returns:
        Number of encoder-layer keys that were successfully loaded.
        (0 means encoder is random; caller should switch to full_finetune)
    """
    encoder_keys_loaded = 0
    try:
        ...
        pretrained_sd = st.load_file(weight_path)
    except ImportError:
        try:
            from huggingface_hub import hf_hub_download
            weight_path = hf_hub_download(
            ...
            pretrained_sd = torch.load(weight_path, map_location="cpu")
        except Exception:
            raise

    # Build explicit key mapping: pretrained -> our model
    # Component name mapping (momentfm names -> our names)
    component_map = {
        "normalizer": "revin",
        "tokenizer": "patch_embedding",
        "head": "reconstruction_head",
    }

    our_sd = model.state_dict()
    new_sd = {}
    matched, skipped, encoder_skipped = 0, 0, 0

    for pre_key, pre_val in pretrained_sd.items():
        # Strip common prefixes like "moment." or "model."
        clean_key = pre_key
        for prefix in ["moment.", "model.", "backbone."]:
        ...
        for old_name, new_name in component_map.items():
            if mapped_key.startswith(old_name + "."):
                mapped_key = new_name + mapped_key[len(old_name):]
                break

        # Try to find in our state dict
        if mapped_key in our_sd and our_sd[mapped_key].shape == pre_val.shape:
            new_sd[mapped_key] = pre_val
            matched += 1
            if "encoder" in mapped_key:
                encoder_keys_loaded += 1
        else:
            # Try suffix match as last resort
            found = False
            for our_key, our_val in our_sd.items():
                our_suffix = our_key.rsplit(".", 1)[-1] if "." in our_key else our_key
                pre_suffix = mapped_key.rsplit(".", 1)[-1] if "." in mapped_key else mapped_key
                if (our_suffix == pre_suffix and
                        our_val.shape == pre_val.shape and
                        our_key not in new_sd):
                    new_sd[our_key] = pre_val
                    matched += 1
                    found = True
                    break
            if not found:
                if "encoder" in pre_key or "block" in pre_key:
                    encoder_skipped += 1
                skipped += 1
    ...
    logger.info(f"Weight loading: {matched} keys matched, {skipped} skipped "
                f"(encoder keys not transferable: {encoder_skipped})")

    if encoder_skipped > 10:
        logger.warning(
            "=" * 70 + "\n"
            "WARNING: Encoder weights could NOT be loaded!\n"
            "Our model uses nn.TransformerEncoder but pre-trained model uses T5.\n"
            "These architectures are incompatible for direct weight transfer.\n\n"
            "For BEST results, install momentfm:\n"
            "    pip install momentfm\n\n"
            "Without momentfm, the encoder is randomly initialized.\n"
            "Will auto-switch to full_finetune mode for training.\n"
            + "=" * 70
        )

    return encoder_keys_loaded
...
class Trainer:
    ...
    def _train_epoch(self, loader, optimizer, scheduler, scaler, amp_dtype):
        """Single training epoch with raw-space masked reconstruction loss."""
        self.model.train()
        total_loss = 0
        num_batches = 0
        ...
        for batch in loader:
            ...
            with torch.amp.autocast(self.device, dtype=amp_dtype, enabled=use_amp):
                ...
                # FIX C: Use raw-space (RobustScaler space) MSE for training.
                # Previously used RevIN-normalized space which explodes with
                # random init (70M loss). Raw-space MSE is:
                #  1. Same scale as validation loss -> comparable metrics
                #  2. Bounded even with random init -> stable training
                #  3. Interpretable -> "how far off is the reconstruction?"
                recon = output["reconstruction"]  # Denormalized by RevIN

                # Build combined mask: observed AND masked-for-prediction
                patch_mask = output["patch_mask"]  # (B, N) -- 1=keep, 0=mask
                masked_indicator = (1 - patch_mask)  # 1 where model must predict

                # Expand patch mask to timestep level
                mask_ts = masked_indicator.unsqueeze(-1).repeat(
                    1, 1, self.config.patch_len
                ).reshape(ts.shape[0], 1, -1)  # (B, 1, T)

                # Combined mask: only compute loss on observed+masked timesteps
                combined_mask = mask * mask_ts  # (B, 1, T)

                # Masked reconstruction loss (only on patches the model predicted)
                diff = (recon - ts) ** 2
                n_masked = combined_mask.sum().clamp(min=1.0)
                masked_loss = (diff * combined_mask).sum() / n_masked

                # Small regularization loss on unmasked patches for stability
                unmasked_mask = mask * (1 - mask_ts)
                n_unmasked = unmasked_mask.sum().clamp(min=1.0)
                unmasked_loss = (diff * unmasked_mask).sum() / n_unmasked

                loss = masked_loss + 0.1 * unmasked_loss

            # Handle scaler=None on CPU
            if use_amp:
...
def run_pipeline(config: MOMENTConfig):
    ...
    model, encoder_loaded = load_moment_pretrained(config)

    # FIX B: Auto-switch to full_finetune if encoder is random
    # Linear probe with a random encoder is useless -- the frozen garbage
    # embeddings give the head nothing meaningful to learn from
    if not encoder_loaded and config.finetune_mode == "linear_probe":
        logger.warning(
            "Encoder weights are RANDOM (not pre-trained).\n"
            "Linear probe mode freezes random features -> terrible results.\n"
            "AUTO-SWITCHING to full_finetune mode so all layers can learn.\n"
            "For linear probe to work, install momentfm: pip install momentfm"
        )
        config.finetune_mode = "full_finetune"
        # Increase LR for training from scratch (default 1e-4 is for fine-tuning)
        if config.learning_rate < 5e-4:
            config.learning_rate = 1e-3
            logger.info(f"Adjusted learning rate to {config.learning_rate} for from-scratch training")
        # Encoder needs same LR as head when training from scratch
        # (differential LR is only for preserving pre-trained encoder features)
        config.encoder_lr_factor = 1.0
    ...
    # FIX D: Normalize anomaly scores to [0,1] per channel using training statistics.
    # Raw MSE scores vary wildly across channels (pH MSE: 81, FRC MSE: 420, Sulphates: 0.2)
    # making thresholds and alert levels meaningless. Percentile normalization
    # ensures scores are comparable and the MVP alert thresholds (0.4/0.7) work.
    num_channels = train_scores["channel_scores"].shape[1]
    score_norms = {}  # store per-channel normalization params
    for ch in range(num_channels):
        ch_train = train_scores["channel_scores"][:, ch, :]
        p1 = np.percentile(ch_train, 1)
        p99 = np.percentile(ch_train, 99)
        score_norms[ch] = (p1, p99)
        if p99 > p1 + 1e-8:
            train_scores["channel_scores"][:, ch, :] = np.clip(
                (ch_train - p1) / (p99 - p1), 0, 1
            )
            ch_test = test_scores["channel_scores"][:, ch, :]
            test_scores["channel_scores"][:, ch, :] = np.clip(
                (ch_test - p1) / (p99 - p1), 0, 1
            )
    # Recompute global scores from normalized channel scores
    train_scores["global_scores"] = train_scores["channel_scores"].mean(axis=1)
    test_scores["global_scores"] = test_scores["channel_scores"].mean(axis=1)
    logger.info("Anomaly scores normalized to [0,1] per channel")
