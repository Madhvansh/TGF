...
class WaterQualityDomain:
    ...
    PARAMETER_SPEC: Dict[str, Tuple[str, float, float, str, bool]] = {
        ...
        "Conductivity_uS_cm":       ("Conductivity",      0.0,   3000.0,"µS/cm",   True),
...
class CoolingTowerDataset(Dataset):
    ...
    Implements MOMENT's data handling strategy:
        ...
        4. Observation mask M ∈ {0,1}^T for handling missing values
    ...
    def __len__(self):
        # Each window × each channel (channel independence)
...
class DataPipeline:
    ...
    def load_and_preprocess(self, data_path: Optional[str] = None) -> Tuple[
        ...
        # 1 = observed, 0 = missing (per paper: M = {0,1}^{1×T})
        ...
        test_dataset = CoolingTowerDataset(
            ...
            max(self.config.stride // 2, 1), self.column_names  # finer stride for test
        )
...
class PatchEmbedding(nn.Module):
    ...
    designated learnable mask embedding [MASK] ∈ R^{1×D}, otherwise."
    """
...
def load_moment_pretrained(config: MOMENTConfig) -> MOMENT:
    ...
    if config.use_pretrained:
        try:
            ...
            # Transfer weights from pre-trained model to our wrapper
            state_dict = pretrained_model.state_dict()
            missing, unexpected = model.load_state_dict(state_dict, strict=False)
            logger.info(f"Loaded pre-trained weights. "
                        f"Missing: {len(missing)}, Unexpected: {len(unexpected)}")
            if missing:
                logger.info(f"Missing keys (will use random init): "
...
def get_optimizer(model: MOMENT, config: MOMENTConfig):
    ...
    Paper Section 3.3: Adam with weight decay, β1=0.9, β2=0.999
...
class Trainer:
    ...
    def train(
        ...
        # Mixed precision (bfloat16 per paper, fallback to float16)
        amp_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
        scaler = torch.amp.GradScaler("cuda", enabled=(self.config.use_amp and self.device == "cuda"))
    ...
    def _train_epoch(self, loader, optimizer, scheduler, scaler, amp_dtype):
        ...
        for batch in loader:
            ...
            with torch.amp.autocast(self.device, dtype=amp_dtype, enabled=self.config.use_amp):
                ...
                target = self.model.revin(ts, mask, mode="norm")
    ...
    def _validate_epoch(self, loader, amp_dtype):
        ...
        for batch in loader:
            ...
            with torch.amp.autocast(self.device, dtype=amp_dtype, enabled=self.config.use_amp):
                ...
                loss = F.mse_loss(recon * mask.unsqueeze(1) if mask.dim() == 2
                                  else recon * mask,
                                  ts * mask.unsqueeze(1) if mask.dim() == 2
                                  else ts * mask)
        ...
        with torch.no_grad():
            for ch_idx, col in enumerate(self.data_pipeline.column_names):
                ...
                anomaly_scores[col] = {
                    ...
                    "is_anomalous": bool(
                        score.mean() > self.anomaly_engine.channel_thresholds.get(
                            ch_idx, POTThreshold()
                        ).threshold if ch_idx in self.anomaly_engine.channel_thresholds
                        else False
                    ),
                }
...
def setup_logging(config: MOMENTConfig) -> logging.Logger:
    ...
    return logging.getLogger("MOMENT-TGF")
