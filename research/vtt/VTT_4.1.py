    ...
    def __init__(self, data, window_size=100, stride=50, mode='train', scaler=None):
        ...
        if scaler is None:
            ...
            if mode == 'train':
                ...
                # If no scaler provided and mode is test, we still need to fit
                self.normalized_values = self.scaler.fit_transform(self.values)
        else:
            self.scaler = scaler
...
class VTTAnomalyDetector:
    ...
    def train(self, train_df, val_df=None):
        """Train the VTT model"""
        # Create train dataset first (this fits the scaler)
        train_dataset = CoolingTowerDataset(
            train_df, 
            window_size=self.config['window_size'],
            stride=self.config['stride'],
            mode='train',
            scaler=None  # Let it create and fit a new scaler
        )
        
        # Store the fitted scaler
        self.scaler = train_dataset.scaler
        
        # Create validation dataset with the fitted scaler
        if val_df is not None:
            val_dataset = CoolingTowerDataset(
                ...
                stride=self.config['stride'],
                mode='test',
                scaler=self.scaler  # Pass the fitted scaler
            )
    ...
    def detect_anomalies(self, test_df, threshold=None):
        ...
        test_dataset = CoolingTowerDataset(
            ...
            stride=self.config['stride'],
            mode='test',
            scaler=self.scaler  # Use the stored scaler
        )
