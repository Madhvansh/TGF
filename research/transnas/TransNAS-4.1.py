    ...
    def __init__(self, config: Dict):
        ...
        self.encoder_layers = nn.ModuleList([
            TransformerBlock(
                ...
                d_ff=config.get('dim_feedforward', 128) * 2,
    ...
    def time_warping(x, sigma=0.2):
        ...
        for b in range(batch_size):
            for f in range(n_features):
                warped[b, :, f] = np.interp(time_points, warp, x[b, :, f])
        
        return torch.tensor(warped, dtype=x.dtype)
...
class EvaluationMetrics:
    ...
    def calculate_EACS(f1_score, training_time, param_count, 
        ...
        Args:
            f1_score: Model's F1 score
            training_time: Training time in seconds
            param_count: Number of model parameters
            f1_max: Maximum F1 score among all models
            time_max: Maximum training time among all models
            param_max: Maximum parameter count among all models
            wa: Weight for accuracy (default 0.4)
            wt: Weight for time efficiency (default 0.4)  
            wp: Weight for model complexity (default 0.2)
        """
    ...
    def calculate_F1PAK(y_true, y_pred, anomaly_scores, K_values=None):
        ...
        Args:
            ...
            K_values: List of K thresholds (0-100)
        """
        if K_values is None:
...
class TransNASOptimizer:
    ...
    def train_and_evaluate(self, config, trial, epochs=50):
...
class TransNASPipeline:
    ...
    def run_nas_optimization(self, train_data, val_data, n_trials=100):
    ...
    def train_final_model(self, config, train_data, val_data, epochs=100):
    ...
    def run_complete_pipeline(self, data_path: str, n_trials: int = 100):
...
def main():
    ...
    results, history, config, pareto_front = pipeline.run_complete_pipeline(
        ...
        n_trials=100  # Number of NAS trials (can be reduced for faster testing)
    )
