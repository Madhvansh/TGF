...
class CoolingTowerDataset(Dataset):
    ...
    def _create_sequences(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Warn if no sequences created
        if len(sequences) == 0:
            logger.warning(
                f"No sequences created! Data length ({len(self.data)}) is smaller than seq_len ({seq_len}). "
                f"Consider reducing seq_len in config."
            )
            # Return empty arrays with correct shape
            return np.array([]).reshape(0, seq_len, self.data.shape[1]), np.array([]).reshape(0, seq_len, self.data.shape[1])
...
class MOMENTAnomalyDetector:
    ...
    def train(
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,
            num_workers=0,
            drop_last=False  # Changed to False to avoid losing data with small datasets
        )
    ...
    def _fit_threshold(self, train_loader: DataLoader):
        ...
        # Safety check: ensure we have scores to fit threshold
        if len(all_scores) == 0:
            logger.warning("No scores collected for threshold fitting. Using default threshold.")
            self.thresholder.threshold = 0.5  # Default fallback
        else:
...
def main():
    ...
    # Configuration
    config = MOMENTConfig(
        seq_len=64,            # Reduced from 512 to work with smaller datasets
